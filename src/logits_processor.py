...
@torch.compile
def logit_adjustment_no_transform(
    log_alpha_prev: torch.Tensor,
    log_A: torch.Tensor,
    log_B: torch.Tensor,
    expectation_zm: torch.Tensor,
    product_generated_toxicity: torch.Tensor,
    ...
    """
    CTRL-G adjustment WITHOUT the sigmoid-logit reshaping (decode-time transform).
    Uses raw EAP directly for reweighting instead of sigmoid(a * logit(EAP)).
    Used for ablation studies (Table 2).
    """
    # Steps 1-3: identical to logit_adjustment
    log_p_zm_x_less_m = torch.vmap(stable_mvm, in_dims=(None, 0))(log_A.squeeze(0).T, log_alpha_prev)
    log_p_x = torch.vmap(stable_mvm, in_dims=(None, 0))(log_B.T, log_p_zm_x_less_m)
    log_expectation_zm_x_less_m = log_p_zm_x_less_m + torch.log(expectation_zm + epsilon)
    log_expectation_xm = torch.vmap(stable_mvm, in_dims=(None, 0))(log_B.T, log_expectation_zm_x_less_m) - log_p_x
    log_expectation_xm += torch.log(product_generated_toxicity.unsqueeze(1) + epsilon) + torch.log(exp_weights + epsilon)

    # Skip sigmoid-logit scaling — use raw EAP directly
    expectation_xm = torch.exp(log_expectation_xm)
    expectation_xm = torch.clamp(expectation_xm, epsilon, 1.0)

    # Re-weight language model probabilities and normalize
    p_lm = torch.softmax(scores, dim=-1)
    p_adjusted = p_lm * expectation_xm
    p_adjusted = p_adjusted / (p_adjusted.sum(dim=-1, keepdim=True) + epsilon)
    adjusted_logits = torch.log(p_adjusted + epsilon)
...
def _compute_eap_for_dump(
    ...
    """
    Compute per-token EAP (pre- and post-transform) for dumping.
    NOT compiled — only called once on step 0 when dump_eap_path is set.
...
class HmmGuidedLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # Dump per-token EAP on first generation step if requested
            if self.dump_eap_path and self.generation_step == 0 and not self._eap_dumped:
                eap_pre, eap_post = _compute_eap_for_dump(
            ...
            # Dispatch to the appropriate compiled function
            if self.decode_transform:
                ...
                adjusted_logits = logit_adjustment_no_transform(
                    log_alpha_prev=self.log_alpha_prev,
                    log_A=self.log_A,
                    log_B=self.log_B,
                    expectation_zm=expectation_zm,
